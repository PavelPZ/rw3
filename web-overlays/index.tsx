...
export enum ModalType { modal, popup }
...
class OverlaysStack extends React.Component<{}, IOverlaysStackState> {
  ...
  render(): JSX.Element {
    ...
    return <div id={providerOverlayId} onKeyDown={ev => this.onGlobalKeyDown(ev)} tabIndex={0} className={renderCSS({ outline: 'none', position: 'absolute', top: 0, left: 0, right: 0, bottom: 0 })}>
      {stack.map((st, idx) => st.$type == ModalType.modal ? <ModalWrapper $idx={idx} key={idx} /> : <PopupWrapper $idx={idx} key={idx} />)}
    </div>;
  }
...
abstract class Wrapper extends React.Component<{ $idx: number; }> {
  ...
  abstract doRender(par: { delay: number; $uniqueId: number; zIndex: number; overlayBackground: string; }, content: JSX.Element): JSX.Element;
  ...
  render(): JSX.Element {
    ...
    return this.doRender({ delay, $uniqueId, zIndex, overlayBackground },
...
class PopupWrapper extends Wrapper {
  ...
  doRender(par: { delay: number; $uniqueId: number; zIndex: number; overlayBackground: string; }, content: JSX.Element): JSX.Element {
    const { delay, $uniqueId, zIndex, overlayBackground } = par;
